...
export const isValidDistancePoint = (point: DistancePoint): boolean => {
  ...
  const hasValidAccuracy =
    ...
    point.accuracy === null ||
    (isFiniteNumber(point.accuracy) && point.accuracy <= MAX_ACCEPTED_ACCURACY_METERS);
...
export const calculateTotalDistance = (points: readonly DistancePoint[]): number => {
  ...
  return validPoints.reduce((totalDistance, point, index) => {
    ...
    return totalDistance + calculateDistanceBetweenPoints(validPoints[index - 1], point);
  }, 0);
};
